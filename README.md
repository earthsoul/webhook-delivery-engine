...
- **No rate limiting on subscription / event creation.** A single client could spam-create thousands of subscriptions or pump millions of events. The natural fix here is the **companion [rate-limiter-service](https://github.com/earthsoul/rate-limiter-service)** — a sibling project in this portfolio that does exactly this, configurable per-route. Wiring it up is a small operational change (a single `POST /api/check` per inbound request); the architecture is already designed to compose cleanly with it.
